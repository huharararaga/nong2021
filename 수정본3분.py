...
if any:
    ...
    def rsiindex(symbol):
        url = "https://api.upbit.com/v1/candles/minutes/3" #### minutes 뒤에 원하는 분 설정을 넣으면 된다 기본적으로 3분으로 설정
        ...
        if rsi<30:     ######## rsi 지수가 30미만이면 텔레그램봇을통해 메시지 전송 ,원하는 숫자로 수정가능
            bot.sendMessage(chat_id=chat_id, text=f"{symbol}:rsi:{round(rsi,3)}  /3분봉 ") 
        time.sleep(0)
